...
class RollingBacktester:
    ...
    def run(self, df: pd.DataFrame) -> dict:
        ...
        if self.start_date:
            ...
            self.start_date = (
                dates[-self.window_days] if len(dates) > self.window_days else dates[0]
            )
        ...
        yearly_breakdown = (
            ...
            .agg({"hit_top_5": "mean", "hit_top_10": "mean", "actual": "count"})
            .rename(columns={"actual": "total_days"})
            .to_dict("index")
        )
        ...
        logger.info(
            f"Backtest complete. Total Days: {metrics['total_days']}, Top 5: {hit_rate_top_5:.2%}, Top 10: {hit_rate_top_10:.2%}"
        )
        return metrics

...
class HeatModel:
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        recent_freq = (
            recent_df["jodi"].value_counts(normalize=True).reindex(self.all_jodis, fill_value=0)
        )

        # Long Term Frequency
        long_term_df = df[df["date"] > latest_date - pd.Timedelta(days=self.long_term_window)]
        long_term_freq = (
            long_term_df["jodi"].value_counts(normalize=True).reindex(self.all_jodis, fill_value=0)
        )

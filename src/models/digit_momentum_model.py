...
class DigitMomentumModel:
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        open_digits = recent_df['open_digit'].value_counts(normalize=True).reindex(range(10), fill_value=0)
        close_digits = recent_df['close_digit'].value_counts(normalize=True).reindex(range(10), fill_value=0)

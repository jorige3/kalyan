...
def compute_jodi_matrix(df, last_n_days=60):
    ...
    df["open_panel_last"] = (
        df["open_panel"].astype(str).str[-1].fillna("0").replace("", "0").astype(int)
    )
    df["close_panel_last"] = (
        df["close_panel"].astype(str).str[-1].fillna("0").replace("", "0").astype(int)
    )
    ...
    analysis_df = df[df["date"] > analysis_start_date].dropna(
        subset=["prev_open_panel_last", "prev_close_panel_last"]
    )
    ...
    open_freq = (
        analysis_df["open_digit"].value_counts(normalize=True).reindex(range(10), fill_value=0)
    )
    close_freq = (
        analysis_df["close_digit"].value_counts(normalize=True).reindex(range(10), fill_value=0)
    )

    # 2. Conditional Probabilities P(digit_t | prev_panel_last_{t-1})
    # Using crosstab to get P(digit | prev_panel)
    cond_open = pd.crosstab(
        analysis_df["prev_open_panel_last"], analysis_df["open_digit"], normalize="index"
    )
    cond_close = pd.crosstab(
        analysis_df["prev_close_panel_last"], analysis_df["close_digit"], normalize="index"
    )

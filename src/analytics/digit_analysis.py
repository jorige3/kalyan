...
class DigitAnalyzer:
    ...
    def get_frequencies(self, window=30) -> dict:
        ...
        open_freq = latest_df['open_digit'].value_counts(normalize=True).reindex(range(10), fill_value=0).to_dict()
        close_freq = latest_df['close_digit'].value_counts(normalize=True).reindex(range(10), fill_value=0).to_dict()
